...
object ClientOptions {
  ...
  def apply(
    ...
    val rv = new MutableClientOptions {}
    clientId.foreach(_ => rv.clientId = clientId)
    connectTimeout.foreach(_ => rv.connectTimeout = connectTimeout)
    createWebsocket.foreach(_ => rv.createWebsocket = createWebsocket)
    defaultProtocol.foreach(_ => rv.defaultProtocol = defaultProtocol)
    encoding.foreach(_ => rv.encoding = encoding)
    host.foreach(_ => rv.host = host)
    keepalive.foreach(_ => rv.keepalive = keepalive)
    log.foreach(_ => rv.log = log)
    manualConnect.foreach(_ => rv.manualConnect = manualConnect)
    my.foreach(_ => rv.my = my)
    objectMode.foreach(_ => rv.objectMode = objectMode)
    password.foreach(_ => rv.password = password)
    path.foreach(_ => rv.path = path)
    port.foreach(_ => rv.port = port)
    protocol.foreach(_ => rv.protocol = protocol)
    protocolId.foreach(_ => rv.protocolId = protocolId)
    protocolVersion.foreach(_ => rv.protocolVersion = protocolVersion)
    query.foreach(_ => rv.query = query)
    queueQoSZero.foreach(_ => rv.queueQoSZero = queueQoSZero)
    reconnectPeriod.foreach(_ => rv.reconnectPeriod = reconnectPeriod)
    reschedulePings.foreach(_ => rv.reschedulePings = reschedulePings)
    resubscribe.foreach(_ => rv.resubscribe = resubscribe)
    servername.foreach(_ => rv.servername = servername)
    timerVariant.foreach(_ => rv.timerVariant = timerVariant)
    transformWsUrl.foreach(_ => rv.transformWsUrl = transformWsUrl)
    username.foreach(_ => rv.username = username)
    writeCache.foreach(_ => rv.writeCache = writeCache)
    wsOptions.foreach(_ => rv.wsOptions = wsOptions)
    rv
}
